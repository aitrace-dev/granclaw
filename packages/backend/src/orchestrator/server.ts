...
export function createServer() {
  ...
  /**
   * Stream a session's WebM recording with HTTP range support so the browser's
   * <video> tag can seek. Returns 404 if the session has no recording or the
   * file is corrupt (magic-byte check in getVideoPath).
   */
  app.get('/agents/:id/browser-sessions/:sessionId/video', (req, res) => {
    const managed = getManagedAgent(req.params.id);
    if (!managed) { res.status(404).json({ error: 'Agent not found' }); return; }
    const filePath = getVideoPath(req.params.id, req.params.sessionId);
    if (!filePath) { res.status(404).json({ error: 'Recording not available' }); return; }
  ...
  httpServer.on('upgrade', (req, socket, head) => {
    // Live browser screencast relay — /browser-live/:agentId/:sessionId
    const liveHandled = handleBrowserLiveUpgrade(req, socket, head, (agentId) => {
      const managed = getManagedAgent(agentId);
      if (!managed) return null;
      return path.resolve(REPO_ROOT, managed.config.workspaceDir);
    });
